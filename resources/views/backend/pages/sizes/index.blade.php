...
@section('admin-content')
    ...
    <div class="container-fluid">
        @include('backend.pages.sizes.partials.top-show')
...
$(document).ready(function() {
    const ajaxURL = "<?php echo Route::is('admin.sizes.trashed' ? 'sizes/trashed/view' : 'sizes') ?>";
    ...
    $('#deleteSelected').on('click', function() {
        ...
        if (selected.length > 0) {
            if (confirm('Are you sure you want to delete the selected records?')) {
                $.ajax({
                    url: '{{ route("admin.sizes.delete.multiple") }}',
                    type: 'POST',
                    data: {
                        ids: selected,
                        _token: '{{ csrf_token() }}'
