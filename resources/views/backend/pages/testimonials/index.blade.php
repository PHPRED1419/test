...
@section('admin-content')
    ...
    <div class="container-fluid">
        @include('backend.pages.testimonials.partials.top-show')
        @include('backend.layouts.partials.messages')

        <button id="deleteSelected" class="btn btn-danger mb-3">Delete Permanently</button>
        <div class="table-responsive product-table">
            <table class="table table-striped table-bordered display ajax_view" id="testimonials_table">
                <thead>
                    <tr>
                        <th width="7%" class="no-sort" data-orderable="false"><input type="checkbox" id="select-all"></th>
                        <th width="7%">Sl</th>
                        <?php /*<th width="21%">Image</th> */ ?>
...
$(document).ready(function() {
    const ajaxURL = "<?php echo Route::is('admin.testimonials.trashed' ? 'testimonials/trashed/view' : 'testimonials') ?>";
    const table = $('table#testimonials_table').DataTable({
        dom: 'Blfrtip',
    ...
    $('#deleteSelected').on('click', function() {
        ...
        if (selected.length > 0) {
            if (confirm('Are you sure you want to delete the selected records?')) {
                $.ajax({
                    url: '{{ route("admin.testimonials.delete.multiple") }}',
                    type: 'POST',
                    data: {
                        ids: selected,
                        _token: '{{ csrf_token() }}'
