...
@section('admin-content')
    ...
    <div class="container-fluid">
        @include('backend.pages.product_enquiry.partials.top-show')
...
$(document).ready(function() {
    const ajaxURL = "<?php echo Route::is('admin.product_enquiry.trashed' ? 'product_enquiry/trashed/view' : 'product_enquiry') ?>";
    ...
    $('#deleteSelected').on('click', function() {
        ...
        if (selected.length > 0) {
            if (confirm('Are you sure you want to delete the selected records?')) {
                $.ajax({
                    url: '{{ route("admin.product_enquiry.delete.multiple") }}',
                    type: 'POST',
                    data: {
                        ids: selected,
                        _token: '{{ csrf_token() }}'
