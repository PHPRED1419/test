...
@section('admin-content')
    ...
    <div class="container-fluid">
        @include('backend.pages.contact_enquiry.partials.top-show')
...
$(document).ready(function() {
    const ajaxURL = "<?php echo Route::is('admin.contactus_enquiry.trashed' ? 'contactus_enquiry/trashed/view' : 'contactus_enquiry') ?>";
    ...
    $('#deleteSelected').on('click', function() {
        ...
        if (selected.length > 0) {
            if (confirm('Are you sure you want to delete the selected records?')) {
                $.ajax({
                    url: '{{ route("admin.contactus_enquiry.delete.multiple") }}',
                    type: 'POST',
                    data: {
                        ids: selected,
                        _token: '{{ csrf_token() }}'
