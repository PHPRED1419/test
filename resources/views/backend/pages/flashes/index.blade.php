...
@section('admin-content')
    ...
    <div class="container-fluid">
        @include('backend.pages.flashes.partials.top-show')
        @include('backend.layouts.partials.messages')

        <button id="deleteSelected" class="btn btn-danger mb-3">Delete Permanently</button>
        <div class="table-responsive product-table">
            <table class="table table-striped table-bordered display ajax_view" id="flashs_table">
                <thead>
                    <tr>
...
$(document).ready(function() {
    const ajaxURL = "<?php echo Route::is('admin.flashes.trashed' ? 'flashes/trashed/view' : 'flashes') ?>";
    const table = $('table#flashs_table').DataTable({
        dom: 'Blfrtip',
        ...
        columns: [
           ...
           // {data: 'id', name: 'id'},
            ...
            {data: 'flash_image', name: 'flash_image'},
    ...
    $('#deleteSelected').on('click', function() {
        ...
        if (selected.length > 0) {
            if (confirm('Are you sure you want to delete the selected records?')) {
                $.ajax({
                    url: '{{ route("admin.flashes.delete.multiple") }}',
                    type: 'POST',
                    data: {
                        ids: selected,
                        _token: '{{ csrf_token() }}'
