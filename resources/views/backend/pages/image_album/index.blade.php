...
@section('admin-content')
    ...
    <div class="container-fluid">
        @include('backend.pages.image_album.partials.top-show')
        @include('backend.layouts.partials.messages')

        <button id="deleteSelected" class="btn btn-danger mb-3">Delete</button>
        <div class="table-responsive product-table">
            <table class="table table-striped table-bordered display ajax_view" id="image_album_table">
                <thead>
                    <tr>
...
$(document).ready(function() {
    const ajaxURL = "<?php echo Route::is('admin.image_album.trashed' ? 'image_album/trashed/view' : 'image_album') ?>";
    const table = $('table#image_album_table').DataTable({
        dom: 'Blfrtip',
    ...
    $('#deleteSelected').on('click', function() {
        ...
        if (selected.length > 0) {
            if (confirm('Are you sure you want to delete the selected records?')) {
                $.ajax({
                    url: '{{ route("admin.image_album.delete.multiple") }}',
                    type: 'POST',
                    data: {
                        ids: selected,
                        _token: '{{ csrf_token() }}'
