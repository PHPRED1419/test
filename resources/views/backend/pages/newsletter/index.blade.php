...
@section('title')
    @include('backend.pages.newsletter.partials.title')
@endsection

@section('admin-content')
    @include('backend.pages.newsletter.partials.header-breadcrumbs')
    <div class="container-fluid">
        @include('backend.pages.newsletter.partials.top-show')
...
$(document).ready(function() {
    const ajaxURL = "<?php echo Route::is('admin.newsletter.trashed' ? 'newsletter/trashed/view' : 'newsletter') ?>";
    ...
    $('#deleteSelected').on('click', function() {
        ...
        if (selected.length > 0) {
            if (confirm('Are you sure you want to delete the selected records?')) {
                $.ajax({
                    url: '{{ route("admin.newsletter.delete.multiple") }}',
                    type: 'POST',
                    data: {
                        ids: selected,
                        _token: '{{ csrf_token() }}'
