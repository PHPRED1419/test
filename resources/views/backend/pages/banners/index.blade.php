...
@section('admin-content')
    ...
    <div class="container-fluid">
        @include('backend.pages.banners.partials.top-show')
...
$(document).ready(function() {
    const ajaxURL = "<?php echo Route::is('admin.banners.trashed' ? 'banners/trashed/view' : 'banners') ?>";
    const table = $('table#banners_table').DataTable({
        dom: 'Blfrtip',
    ...
    $('#deleteSelected').on('click', function() {
        ...
        if (selected.length > 0) {
            if (confirm('Are you sure you want to delete the selected records?')) {
                $.ajax({
                    url: '{{ route("admin.banners.delete.multiple") }}',
                    type: 'POST',
                    data: {
                        ids: selected,
                        _token: '{{ csrf_token() }}'
