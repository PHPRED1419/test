...
if (!function_exists('generateCategoryBreadcrumbs')) {
    function generateCategoryBreadcrumbs($category, $separator = ' &raquo; ', $activeClass = 'active', $heading = null)
    {
        try {
            ...
            // Start building HTML
            $html = '<nav aria-label="breadcrumb" class="breadcrumb_bg">
            ...
            foreach ($breadcrumbs as $index => $crumb) {
                ...
                if (!$isLast) {
                    $html .= '<a href="'.url('category/'.$crumb->slug).'">';
                }
                
                $html .= e($crumb->name); // Escape output for security
            ...
            \Log::error('Breadcrumb generation failed: ' . $e->getMessage());
            return '';
        }
    }
}
...
if (!function_exists('has_child')) {
    function has_child($catid, $condition = [], $domain_id = null)
    {
        if (!$domain_id) {
            // Example: Get from config or auth if needed
            $domain_id = config('app.domain_id'); // or Auth::user()->domain_id;
        }

        $query = DB::table('wl_categories')
            ->where('parent_category_id', $catid);           

        // Apply status condition if not overridden
        if (empty($condition)) {
            $query->where('status', 1);
        } else {
            // Custom condition(s) as associative array
            foreach ($condition as $key => $value) {
                $query->where($key, $value);
            }
        }

        return $query->exists(); // Returns true if at least one child exists
    }
}
...
if (!function_exists('category_breadcrumb')) {
    function category_breadcrumb($catid, $segment = '')
    {
        ...
        // Get the current segment value from the URL if not directly passed
        $segmentValue = Request::segment($segment) ? Request::segment($segment) : '';

        // Fetch the category
        $category = DB::table('wl_categories')
            ->where('id', $catid)
            ->where('status', 1)
            ->first();
        ...
        if ($category) {
            ...
            if (has_child($category->parent_category_id)) {
                $link_url = URL::to('category/'.$category->slug);

                if (!empty($segmentValue) && $category->id == $segmentValue) {
                    $link_cat[] = '<li class="breadcrumb-item active">' . e($category->name) . '</li>';
                } else {
                    $link_cat[] = '<li class="breadcrumb-item"><a href="' . $link_url . '">' . e($category->name) . '</a></li>';
                }

                // Recursive call
                $link_cat[] = category_breadcrumb($category->parent_category_id, $segment);
            } else {
                $link_url = URL::to('category/'.$category->slug);
                $link_cat[] = '<li class="breadcrumb-item"><a href="' . $link_url . '">' . e($category->name) . '</a></li>';
            }
            ...
            $link_cat[] = '<li class="breadcrumb-item"><a href="' . URL::to('/category') . '">Categories</a></li>';
        }

        $link_cat = array_reverse($link_cat);
        return implode('', $link_cat);
    }
}
...
if (!function_exists('category_breadcrumbs')) {
    function category_breadcrumbs($catid, $segment = '', $heading = '')
    {
		...
		<nav aria-label="breadcrumb" class="breadcrumb_bg">
					<div class="container-xxl position-relative">
						<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="'.URL::to('/').'">Home</a></li>'.$category_links;
        

        if (!empty($heading)) {
            $navigation_content .= '<li class="breadcrumb-item active">' . e($heading) . '</li>';
        }
...
if (!function_exists('applicationCategoryBreadcrumbs')) {
   
    function applicationCategoryBreadcrumbs($category, $separator = ' &raquo; ', $activeClass = 'active',$heading=null)
    {		
        if (is_numeric($category)) {
            $category = ApplicationsCategories::findOrFail($category);
        }
        elseif (is_string($category) && !($category instanceof Category)) {
            $category = ApplicationsCategories::where('slug', $category)->firstOrFail();
        }
		//dd($category);
        ...
        foreach ($breadcrumbs as $index => $crumb) {
            ...
            if (!$isLast) {
				...
				$html .= '<a href="'.URL::to('acategory/'.$crumb->slug).'">';
            }
...
if (!function_exists('appliation_has_child')) {
    function appliation_has_child($catid, $condition = [], $domain_id = null)
    {
        if (!$domain_id) {
            // Example: Get from config or auth if needed
            $domain_id = config('app.domain_id'); // or Auth::user()->domain_id;
        ...
        $query = DB::table('wl_applications_categories')
            ->where('parent_category_id', $catid);           
...
if (!function_exists('appliation_category_breadcrumb')) {
    function appliation_category_breadcrumb($catid, $segment = '')
    {
        $link_cat = [];
        ...
        // Fetch the category
        $category = DB::table('wl_applications_categories')
            ->where('id', $catid)
            ->where('status', 1)
            ->first();
        ...
        if ($category) {
            // If the category has a parent
            if (appliation_has_child($category->parent_category_id)) {
                $link_url = URL::to('acategory/'.$category->slug);
                ...
                // Recursive call
                $link_cat[] = appliation_category_breadcrumb($category->parent_category_id, $segment);
            } else {
                $link_url = URL::to('acategory/'.$category->slug);
                $link_cat[] = '<li class="breadcrumb-item"><a href="' . $link_url . '">' . e($category->name) . '</a></li>';
            }
            ...
            // Fallback to general category page
            $link_cat[] = '<li class="breadcrumb-item"><a href="' . URL::to('/acategory') . '">Categories</a></li>';
        }
...
if (!function_exists('appliation_category_breadcrumbs')) {
    function appliation_category_breadcrumbs($catid, $segment = '', $heading = '')
    {
        $category_links = appliation_category_breadcrumb($catid, $segment);
