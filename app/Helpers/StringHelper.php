...
class StringHelper
{
    ...
    public static function createSlug($title, $model, $field, $separator = "-", $isModule = false)
    {
        ...
        // Dynamically get the primary key of the model
        $primaryKey = (new $model)->getKeyName();

        $allSlugs = StringHelper::getRelatedSlugs($slug, $id, $model, $field, $primaryKey);
    ...
    protected static function getRelatedSlugs($slug, $id = 0, $model, $field, $primaryKey = 'id')
    {
        return $model::select("$field")
            ->where("$field", 'like', $slug . '%')
            ->where($primaryKey, '<>', $id) // Use the dynamically fetched primary key
            ->get();
    }
