...
class InstantOfferEnquiryController extends Controller
{
    public $user;
    ...
    public function index($isTrashed = false)
    {
        ...
        $query = InstantOfferEnquiry::orderBy('status', 'asc')
            ->orderBy('id', 'desc');
            ...
            $contactsDataTable = DataTables::of($contacts, $isTrashed)
                ...
                ->addColumn(
                    ...
                    function ($row) use ($isTrashed) {
                        $html = '<a class="btn waves-effect waves-light btn-info btn-sm btn-circle" title="View Contact Details" href="' . route('admin.instant_offer_enquiry.show', $row->id) . '"><i class="fa fa-eye"></i></a>';
                        return $html;
                    }
                )
        ...
        return view('backend.pages.instant_offer_enquiry.index', [
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        if (!empty($ids)) {
            InstantOfferEnquiry::whereIn('id', $ids)->delete();
    ...
    {
        $contact = InstantOfferEnquiry::find($id);
       ...
       // $contact->save();
        ...
        return view('backend.pages.instant_offer_enquiry.show', compact('contact'));
    }
}
