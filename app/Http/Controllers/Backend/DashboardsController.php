...
class DashboardsController extends Controller
{
    ...
    public function index()
    {
        ...
        $count_pages = 0; //count(Page::select('id')->get());
        $count_blogs = count(Blog::select('id')->get());
        $count_categories = count(Category::select('id')->get());
        $count_products = count(Product::select('products_id')->get());
        ...
        return view('backend.pages.dashboard.index', compact('count_pages', 'count_blogs', 'count_admins', 'count_roles', 'count_categories', 'recent_pages', 'recent_blogs','count_products'));
    }
