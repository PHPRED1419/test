...
class ProductEnquiryController extends Controller
{
    ...
    public function index($isTrashed = false)
    {
        ...
        $query = ProductEnquiry::orderBy('status', 'asc')
            ->orderBy('id', 'desc');
            ...
            $contactsDataTable = DataTables::of($contacts, $isTrashed)
                ...
                ->addColumn(
                    ...
                    function ($row) use ($isTrashed) {
                        $html = '<a class="btn waves-effect waves-light btn-info btn-sm btn-circle" title="View Contact Details" href="' . route('admin.product_enquiry.show', $row->id) . '"><i class="fa fa-eye"></i></a>';
                        return $html;
                    }
                )
        ...
        return view('backend.pages.product_enquiry.index', [
    ...
    {
        $contact = ProductEnquiry::find($id);
       ...
       // $contact->save();
        ...
        return view('backend.pages.product_enquiry.show', compact('contact'));
    }
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        if (!empty($ids)) {
            ProductEnquiry::whereIn('id', $ids)->delete();
