...
@if(!empty($data))
    @foreach($data as $row)
        ...
        <figure class="d-table-cell align-middle text-center"><a href="{{ url('category/' . $row->slug) }}" title=" $row->name  }}">
        @if($row->category_image != "")
            <img src="{{ asset('assets/images/categories/' . $row->category_image) }}" alt="" loading="lazy" fetchpriority="low" decoding="async" width="281" height="283">
        @else
